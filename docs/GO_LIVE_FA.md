...
## اگر Eways در گزارش نیست یا «خراب» است
نشانه‌ی اجرای خراب در لاگ: `پاسخ لاگین: status=200 | Location=` **بدون** خط «✅ لاگین موفق! کوکی Aut دریافت شد».
یعنی لاگین واقعی نشده و سایت فقط نمای مهمان را می‌دهد (فهرست بدون قیمت). کد قدیمی این حالت را «سشن معتبر» حساب می‌کرد؛ حالا:
- لاگین بدون کوکی `Aut` تا ۳ بار تکرار و بعد با پیام روشن **شکست** می‌خورد.
- منبعی که بیشتر پیشنهادهایش قیمت ندارد «خراب» (failed) گزارش می‌شود و در سرصفحه‌ی تلگرام با ⚠️ می‌آید.
علت‌های معمول: رمز اشتباه، کپچا/WAF، مسدود یا محدودشدن IP سرورهای GitHub بعد از لاگین‌های پیاپی.
راه‌ها: کمی بعد دوباره اجرا کنید؛ از یک IP دیگر یا با `EWAYS_PROXY` اجرا کنید؛ یا در مرورگر لاگین کنید و کوکی را در secret به نام `EWAYS_COOKIE` (مثل `Aut=...`) بگذارید.
