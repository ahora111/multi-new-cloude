...
class KasraParsSource(Source):
    ...
    def records(self, watchlist):
        ...
        try:
            ...
            # Source instances are short-lived (one pipeline run). Close the session so repeated
            # test/CLI runs do not accumulate idle sockets.
            session = getattr(self.http, "session", None)
            if session is not None and hasattr(session, "close"):
                session.close()


        # Keep only records with stable identity; a source-level duplicate must not become a duplicate offer.
        out = _dedupe(all_records)
        if not out:
            raise RuntimeError("KasraPars returned zero products (site unreachable, blocked, or layout changed)")
